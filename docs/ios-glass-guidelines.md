...
1. **At most 1–2 native platform views per screen** (tab bar, a glass menu, an alert, a segmented control). Never put a platform view in a list row or message bubble.
   The chat screen is the one exception: its floating chrome is six separate native capsules — back, title, header actions, composer field, composer action (send / mic / video), scroll-down. Each is its own `LiquidGlassContainer`, so they read as distinct glass objects rather than one bar.
...
- **Reduce Transparency:** Flutter 3.47 `AccessibilityFeatures` / `MediaQuery` still expose **no** `reduceTransparency` flag. Komet reads `UIAccessibility.isReduceTransparencyEnabled` via `ru.komet.app/accessibility` (+ event channel) into `IosReduceTransparency.enabled`. When on: `AppIosGlass.nativeViews` is false, `GlassBackground` / style chrome use opaque `IosPalette` fills (no `BackdropFilter`, no UiKitViews). Wallpaper blur, wallpaper motion, the spectrum ticker, and mesh animation stop as well (`IosMotion.freezeLiveEffects`). Override with `IosReduceTransparency.debugOverride` in tests.
- **Low Power Mode:** `ru.komet.app/power` reads `ProcessInfo.isLowPowerModeEnabled` into `IosLowPowerMode`. It uses the same still-effects path as Reduce Transparency. Override with `IosLowPowerMode.debugOverride` in tests.
